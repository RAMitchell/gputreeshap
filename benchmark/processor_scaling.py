...
def plot_gpu_results(results):
    plt.clf()
    sns.barplot(x="GPUs", y="Throughput(rows/s)", color="royalblue", data=results)
    plt.tight_layout()
...
def main():
    iterations = 10
    test_rows = 100000
    cpu_df = cpu_experiment("cal_housing-med", test_rows,iterations)
    plot_cpu_results(cpu_df)
    gpu_df = gpu_experiment("cal_housing-med", test_rows,iterations)
